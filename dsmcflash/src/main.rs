    ...
    match args.command {
        Command::Read { file, offset, length } => {
            ...
            let num_sectors = match length {
                Some(len) => ((len + SMC_NAND_BLOCK_SZ as u64 - 1) / SMC_NAND_BLOCK_SZ as u64) as usize,
        ...
        Command::Write { file, offset } => {
            ...
            let num_sectors = (data.len() + SMC_NAND_BLOCK_SZ as usize - 1) / SMC_NAND_BLOCK_SZ as usize;
